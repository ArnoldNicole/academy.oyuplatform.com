...
class FeesController extends Controller
{
    ...
    public function index()
    {
        ...
        $feesTypeData = FeesType::whereSchoolId($school_id)->get();
    ...
    public function edit($id)
    {
        ...
        $classes = $this->class->all(['*'], ['stream', 'medium', 'stream']);
        $feesTypeData = FeesType::whereSchoolId(auth()->user()->school_id)->get();
    ...
    public function feesPaidListIndex()
    {
        ...
        $fees = $this->fees->builder()->where('school_id', $school_id)->select(['id', 'name', 'school_id'])->get();
    ...
    public function feesPaidReceiptPDF($feesPaidId)
    {
        ...
        try {
            ...
            $file_name = "{$student->user->full_name}  Receipt";
    ...
    public function payCompulsoryFeesStore(Request $request)
    {
        ...
        $request->validate([
            ...
            'total_amount_receiving' => 'required|numeric',
        ], [
        ...
        try {
            ...
            if ($request->input('installment_mode')) {
                $amount = $request->input('total_amount_receiving');
            } else {
            ...
            if (empty($feesPaid)) {
                $data_array = [
                    ...
                    'is_used_installment' => $request->input('installment_mode'),
                    'fees_id' => $request->input('fees_id'),
                    'student_id' => $request->input('student_id'),
                    'amount' => $amount,
                ];
                $feesPaidResult = $this->feesPaid->create($data_array);
            } else {
                $feesPaidResult = $this->feesPaid->update($feesPaid->id, [
                    'amount' => $request->input('total_amount_receiving') + $feesPaid->amount,
                    'is_fully_paid' => ($amount + $feesPaid->amount) >= $fees->total_compulsory_fees
                ]);
            }

            if ($request->input('installment_mode') == 1) {
                foreach ($fees->compulsory_fees as $fees_class_type) {
                    $input_name = "compulsory_fees_{$fees_class_type->id}_amount";
                    $amount_paid = $request->input($input_name);

                    if ($amount_paid > 0) {
                        $installment_data = [
                            'name' => $fees_class_type->fees_type_name,
                            'due_date' => today()->endOfDay()->format('Y-m-d H:i:s'),
                            'due_charges' => 0,
                            'session_year_id' => $fees->session_year_id,
                            'school_id' => $fees->school_id,
                            'fees_id' => $fees->id,
                            'fees_type_id' => $fees_class_type->fees_type_id
                        ];

                        $installment = FeesInstallment::create($installment_data);

                        $compulsoryFeeData = [
                            'student_id' => $request->input('student_id'),
                            'type' => 'Installment Payment',
                            'mode' => $request->input('mode'),
                            'cheque_no' => $request->input('mode') == 2 ? $request->input('cheque_no') : null,
                            'amount' => $amount_paid,
                            'due_charges' => null,
