...
class SubscriptionController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            $amount = ceil($subscriptionBill->amount * 100) / 100;
    ...
    public function plan($id)
    {
        ...
        try {
            ...
            if ($package->is_trial) {
                ...
                $bb = intval($settings['billing_cycle_in_days']);
                $end_date = Carbon::now()->addDays(($bb - 1))->format('Y-m-d');
            }
            $subscription_data = [
    ...
    public function generate_bill($id)
    {
        ...
        try {
            ...
            if($bill_cycle_days == 0)
            {
                $bill_cycle_days = 30;
            }
            
            info("Bill Cycle Days is $bill_cycle_days");
            ...
            if ($auto_renewal_plan) {
                ...
                if (!$check_subscription) {
                    ...
                    $settings = app(CachingService::class)->getSystemSettings();
                    $b_days = $settings['billing_cycle_in_days'];
                    if($b_days == ""){
                        $b_days = 30;
                    }
                    $end_date = Carbon::parse($today_date)->addDays(($b_days - 1))->format('Y-m-d');
