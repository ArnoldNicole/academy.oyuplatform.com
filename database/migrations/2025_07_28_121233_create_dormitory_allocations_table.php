...
return new class extends Migration
{
    ...
    public function up(): void
    {
        Schema::create('dormitory_allocations', function (Blueprint $table) {
            ...
            $table->unsignedBigInteger('allocated_by'); // Staff who allocated
            $table->unsignedBigInteger('checked_out_by')->nullable(); // Staff who processed checkout
            $table->timestamps();

            $table->foreign('student_id')->references('id')->on('students')->onDelete('cascade');
            $table->foreign('dormitory_id')->references('id')->on('dormitories')->onDelete('cascade');
            $table->foreign('school_id')->references('id')->on('schools')->onDelete('cascade');
            $table->foreign('allocated_by')->references('id')->on('staff')->onDelete('restrict');
            $table->foreign('checked_out_by')->references('id')->on('staff')->onDelete('restrict');
